    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6 sm:mb-8 space-y-4 lg:space-y-0">
        <div className="flex items-center space-x-4 flex-1 min-w-0">
          ...
          <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900">Edit Note</h1>
        </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        {/* Synopsis */}
        <div>
          <label htmlFor="synopsis" className="block text-xs sm:text-sm font-medium text-gray-700 mb-2">
            Synopsis *
          </label>
          <textarea
            {...register('synopsis', {
          ...
          />
          {errors.synopsis && (
            <p className="mt-1 text-xs sm:text-sm text-red-600">{errors.synopsis.message}</p>
          )}
        </div>
        ...
        <div>
          <label htmlFor="content" className="block text-xs sm:text-sm font-medium text-gray-700 mb-2">
            Content * {!showPreview && <span className="text-gray-500">(Markdown supported)</span>}
          </label>
          
          {showPreview ? (
            <div className="min-h-[300px] sm:min-h-[400px] p-3 sm:p-4 border border-gray-300 rounded-lg bg-gray-50 overflow-auto">
              <div className="prose prose-sm sm:prose-base max-w-none">
                <ReactMarkdown>{watchedContent || 'Nothing to preview yet...'}</ReactMarkdown>
              </div>
            </div>
          ) : (
            <textarea
